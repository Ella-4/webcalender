...
def render_simple_calendar(year, month, conferences):
    ...
    for i, day in enumerate(weekdays):
        with cols[i]:
            color = "#ef4444" if i == 6 else "#22c55e" if i == 5 else "#374151"
            st.markdown(f'<div style="background: #f0fdf4; padding: 1rem; text-align: center; font-weight: 600; color: {color}; border-bottom: 1px solid rgba(34, 197, 94, 0.1);">{day}</div>', unsafe_allow_html=True)
    ...
    for week in cal:
        ...
        for i, day in enumerate(week):
            with cols[i]:
                if day == 0:
                    st.markdown('<div style="background: #fafbfc; min-height: 100px; border-bottom: 1px solid rgba(34, 197, 94, 0.05); border-right: 1px solid rgba(34, 197, 94, 0.05);"></div>', unsafe_allow_html=True)
                else:
                    # 오늘 날짜 확인
                    ...
                    # 배경색 설정
                    bg_color = "#dcfce7" if is_today else "white"
                    text_color = "#166534" if is_today else "#22c55e" if is_weekend else "#374151"
                    if i == 6:  # 일요일
                        text_color = "#ef4444"
                    ...
                    cell_html = f'''
                    <div style="background: {bg_color}; min-height: 100px; padding: 8px; border-bottom: 1px solid rgba(34, 197, 94, 0.1); border-right: 1px solid rgba(34, 197, 94, 0.1); transition: all 0.2s ease;">
...
def main():
    ...
    with st.sidebar:
        ...
        # 커스텀 스타일을 적용한 multiselect - 연두색으로 통일
        st.markdown("""
        <style>
        ...
        div[data-testid="stMultiSelectTag"]:hover {
            background: linear-gradient(135deg, #bbf7d0 0%, #a7f3d0 100%) !important;
        }
        .stMultiSelect .st-emotion-cache-1f3w014 {
            background: linear-gradient(135deg, #dcfce7 0%, #bbf7d0 100%) !important;
            color: #166534 !important;
            border: 1px solid rgba(34, 197, 94, 0.2) !important;
        }
        .stMultiSelect .st-emotion-cache-1f3w014 button {
            color: #166534 !important;
        }
        </style>
        """, unsafe_allow_html=True)
